...
class ZoneGui:
	def __init__(self,zone,building,root):
		...
		# Fan mode
		self.var_fan = tkinter.StringVar()
		self.var_fan.set("Zone "+str(zone)+" fan")
		self.zone_fan = tkinter.Label(self.frame,textvariable=self.var_fan)
		self.zone_fan.pack()
		self.list_fan = tkinter.Listbox(self.frame,exportselection=0,selectmode=SINGLE,height=2)
		self.list_fan.insert(tkinter.END,"on")
		self.list_fan.insert(tkinter.END,"auto")
		self.list_fan.selection_set(tkinter.END)
		self.list_fan.bind('<<ListboxSelect>>',self.select_fan)
		self.list_fan.pack()
		# Cooling deadband
		self.var_cool = tkinter.StringVar()
		self.var_cool.set("Zone "+str(zone)+" cooling deadband")
		self.zone_cool = tkinter.Label(self.frame,textvariable=self.var_cool)
		self.zone_cool.pack()
		self.list_cool = tkinter.Listbox(self.frame,exportselection=0,selectmode=SINGLE,height=15)
		for d in range(1,16):
			self.list_cool.insert(tkinter.END,str(d))
		self.list_cool.selection_set(tkinter.END)
		self.list_cool.bind('<<ListboxSelect>>',self.select_heat_cool)
		self.list_cool.pack()
		# Heating deadband
		self.var_heat = tkinter.StringVar()
		self.var_heat.set("Zone "+str(zone)+" heating deadband")
		self.zone_heat = tkinter.Label(self.frame,textvariable=self.var_heat)
		self.zone_heat.pack()
		self.list_heat = tkinter.Listbox(self.frame,exportselection=0,selectmode=SINGLE,height=15)
		for d in range(1,16):
			self.list_heat.insert(tkinter.END,str(d))
		self.list_heat.selection_set(tkinter.END)
