...
class MasterNode(Agent):
    ...
    def leader_assert(self):
        ...
        for i in range(0, len(self.Nodes)):
            ...
            node.agent.vip.pubsub.publish('pubsub', topic='leader',
    ...
    def RunControl(self):
        ...
        for nodekey in self.Nodes:
            ...
            OrderDesc = sorted(self.Nodes.values(), key=operator.attrgetter('delta'),
                               reverse=True)

            # no of buildings reqd to satisfy reg signal,
            # bcoz bldgs go up or down in 3 kw increments, divide by 3 to get no of
            # bldgs
            ...
            Log.info(
                "No of required bldgs: " + str(ReqBld) + ", regulation need of: " + str(
                    self.Reg[i]))
            ...
            if self.Reg[i] > 0:
                ...
                for node in OrderDesc:
                    ...
                    if count >= ReqBld:
                        break
            if self.Reg[i] < 0:
                # decrease power consumption, aka switch off equipment, starting with
                # lowest temp difference for comfort
            ...
            try:
                node.agent.vip.pubsub.publish(
                    'pubsub', topic='masternode/command', headers=headers,
                    message=msg).get(timeout=60)
