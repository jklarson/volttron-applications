...
class ModelNode(Agent):
    ...
    def setup(self, sender, **kwargs):
        ...
        # set initial state
        # Note that for the purposes of this experiment, only the cooling
        # condition is used.
    ...
    def Update(self):
        ...
        for i in range(0, len(self.masters)):
            ...
            self.masternode.vip.pubsub.publish('pubsub', topic='temperature',
    ...
    def leader_check(self, peer, sender, bus, topic, headers, message):
        if len(self.leader) < self.numNodes:
            ...
            (item for item in self.leader if item["ID"] == message["ID"]).next()[
                "Time"] = message["Time"]
            ...
            # Sort
            # CHANGE self.leader_sorted = sorted(self.leader, key=itemgetter(
            # 'Time'))
            self.leader_sorted = sorted(self.leader, key=itemgetter("ID"),
                                        reverse=True)
    ...
    def ProcessIncomingCommand(self, peer, sender, bus, topic, headers, message):
        ...
        if (self.leader[0]["Time"] == 999 or len(self.leader_sorted) == 0):
            # CHANGE if(self.leader_sorted[0]["ID"] == 999 or len(
            # self.leader_sorted) == 0):
            assert (False)
